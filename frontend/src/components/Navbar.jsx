...
const Navbar = () => {
    ...
    return (
        <div className='position:fixed w-full'>
            ...
            <div className=" h-20 py-4 px-4 flex items-center justify-between border-b border-slate-300 sm:hidden  ">
                <div className='max-w-40 '>
                    <img src={logo} alt="logo" className="max-w-30 bg-transparent self-center" />
                </div>
                <button className="ml-2 bg-blue-500 px-1 py-1 outline-none cursor-pointer rounded-md text-white font-bold text-sm py-2 px-3 self-center cursor-pointer "><a href="tel:+919999999999">+91 9999999999</a>
                </button>
                <div className='ml-5 self-center'>
                    <button type="button" onClick={() => setIsMenu((prev) => !prev)} className='flex items-center cursor-pointer' >
                        {
                            isMenu ? <img src={cross_icon} className=" text-gray-900 w-6 h-6" /> : <img src={menu_icon} className=" text-gray-900 w-6 h-6 " />
                        }
            ...
            <div className="h-18 hidden sm:block sm:flex flex-row items-center justify-around px-6 py-4 border-b border-slate-300">
                ...
                <div className=''>
                    ...
                    <Link to="/login" className="px-4 text-md  hover:text-lg max-w-30  text-gray-600 hover:text-gray-900 cursor-pointer ">
                        <button className='bg-blue-500 outline-none px-10 py-3 rounded-md text-white hover:bg-blue-600'>Login</button>
                    </Link>
                </div>
            </div>
